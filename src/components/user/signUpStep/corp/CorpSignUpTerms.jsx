...
const { CORP_NICE_CERT_WEBVIEW_SCREEN } = constants
...
export default function CorpSignUpTerms() {
	const { navigate } = useNavigation()
	const { state: termsState } = useTermsContext()
	const corpTermsList = termsState.corpTermsList.data?.data || []
	...
	const handleAcceptBtn = () => {
		if (check[0] && check[1] && check[2] === true) {
			...
			navigate(CORP_NICE_CERT_WEBVIEW_SCREEN, { alarmFlag: alarmCheck })
		} else {
	...
	function handleModal(id) {
		setUseModal({ ...useModal, isOpen: true, content: corpTermsList[id].content })
	}
	return (
		<>
			<ScrollView
